...
const SafeAvatar = ({ p }: { p: any }) => {
  ...
  useEffect(() => {
    ...
    const fetchAvatar = async () => {
      ...
      // 1. Search in preloaded properties
      let photo = p.image || p.userPhoto || p.photoURL || p.avatar || (p.custom as any)?.photoURL || (isMe ? authUser?.photoURL : null);

      // 2. Fetch from Firestore if photo not ready
      if (!photo && p.userId) {
        try {
