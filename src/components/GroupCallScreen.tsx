...
const SafeAvatar = ({ p }: { p: any }) => {
  ...
  useEffect(() => {
    ...
    const fetchAvatar = async () => {
      ...
      const isStreamAutoAvatar = (url?: string) => {
        if (!url) return false;
        return url.includes('getstream.io') || url.includes('ui-avatars.com') || url.includes('stream-io-cdn');
      };

      // 1. Check user custom photo first
      let photo = p.photoURL || p.userPhoto || (p.custom as any)?.photoURL || (isMe ? (authUser?.photoURL || localStorage.getItem('user_photo') || localStorage.getItem('userProfilePhoto')) : null);

      // 2. If no custom photo yet and p.image is not a stream auto-generated default avatar, use p.image
      if (!photo && p.image && !isStreamAutoAvatar(p.image)) {
        photo = p.image;
      }

      // 3. Fetch real user photo from Firestore if not ready
      if (!photo && p.userId) {
        try {
          const d = await getDoc(doc(db, 'users', p.userId));
          if (d.exists()) {
            const data = d.data();
            photo = data.photoURL || data.image || data.avatar || data.userPhoto;
          }
        } catch (e) {}
      }
  ...
  }, [p?.userId, p?.image, p?.userPhoto, p?.photoURL, p?.avatar]);
  ...
  return (
    <div className="w-full h-full bg-gradient-to-tr from-indigo-600 via-purple-600 to-violet-600 flex items-center justify-center text-white font-black text-xs select-none shadow-inner">
      {initial && initial !== 'M' ? initial : <User className="w-1/2 h-1/2 text-white/90" />}
    </div>
  );
};
import { 
