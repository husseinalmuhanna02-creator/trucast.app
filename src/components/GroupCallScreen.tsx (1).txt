  ...
  useEffect(() => {
    ...
    const requestMediaPermissions = async () => {
      try {
        ...
        console.warn("Could not request camera and microphone together. Trying audio only.", err);
        try {
          if (typeof navigator !== 'undefined' && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          ...
          console.warn("Audio permission request failed:", audioErr);
        }
      }
    };
    ...
    const initStream = async () => {
      try {
        // Request permissions programmatically before starting connection
